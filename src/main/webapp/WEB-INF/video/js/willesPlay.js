$(function() {
	...
	$(window).keyup(function(event) {
		...
		if (event.keyCode == 27) {
			...
			setTimeout(function() {
				$('#willesPlay .playControll').hide();
			}, 5000);
		}
		event.preventDefault();
	});

	$('#willesPlay').on('mousemove', function() {
		if ($(this).hasClass('cancleScreen')) {
			$('#willesPlay .playControll').show();
			$('#willesPlay').css('cursor', 'pointer');
			setTimeout(function() {
				$('#willesPlay .playControll').hide();
				$('#willesPlay').css('cursor', 'blank');
			}, 5000);
		}
	});
